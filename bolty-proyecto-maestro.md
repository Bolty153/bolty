...
>
> Última actualización: **17/07/2026** — **EL CEREBRO YA TIENE MANOS (tanda 1, LEER)**: además de la conexión segura (capa 1), la personalidad (capa 3) y la Bandeja Unificada con chat web + agente automático + freno manual + tokens guardados (capa 2), ahora el agente **consulta la base con herramientas (tool use)** en vez de tener todo el catálogo en el prompt: `buscar_productos`, `consultar_stock`, `buscar_servicios`, `consultar_disponibilidad` y `buscar_empleados`. Sabe **qué día/hora es en Argentina** (entiende "mañana", "el viernes", "la semana que viene") y matchea nombres de empleados **sin tildes ni mayúsculas**. La Bandeja se agrandó a **pantalla completa**, permite **eliminar mensajes y conversaciones**, y **Conversaciones + Canales** se movieron a un grupo propio del menú ("Atención"), fuera de "Servicios". Verificado contra el código real. Pendiente: las manos **tanda 2 (ESCRIBIR: agendar de verdad)**, el **panel de tokens/costo**, la **derivación inteligente a humano**, la **fase multimodal** y el **entrenador conversacional** (ver 3.10 y 6.1).
...
- **Ya consulta la base con "manos" (tools):** stock real, precios, servicios, disponibilidad de turnos y el equipo — con herramientas, en vez de tener todo el catálogo metido en el prompt (resuelve el "problema del catálogo grande", 6.1). Entiende fechas relativas ("mañana", "el viernes") con la hora de Argentina.
- **Agenda turnos** para negocios de servicios (sabiendo qué empleado hace qué servicio y a qué hora está libre) — hoy los CONSULTA; **agendar de verdad (escribir el turno) es la tanda 2, pendiente**.
...
### 3.10 🧠 EL CEREBRO — capas 1, 2, 3 y 4-tanda-1 (LEER) — commits `51a038a`, `824217b`, `0ee8c57` + trabajo en curso
...
- **Capa 2 — la Bandeja Unificada (HECHO, `0ee8c57`):** el inbox omnicanal, **una sección del menú** ("Conversaciones", `src/views/Bandeja.tsx`, ruta `bandeja` en `App.tsx`, ítem en `Sidebar.tsx`). Layout de dos columnas: lista de conversaciones a la izquierda, hilo abierto a la derecha (globos + composer). Cada conversación lleva su **ícono de canal** (hoy todas `web`, pero WhatsApp/Instagram/Mail ya están cableados en el tipo). Lo que ya hace:
  ...
  - **Tablas nuevas** (en `schema.sql` sección 25): `conversations` (channel, customer_name, status, **mode** auto/manual, last_message_at) y `messages` (role customer/agent/human, content, tokens_in/out), ambas con RLS por dueño + policy admin (modo soporte) y agregadas a la publicación de Realtime. Hooks: `useConversations.ts`, `useMessages.ts`.
  - **Eliminar (agregado 17/07):** se puede **borrar una conversación entera** (tacho en cada ítem de la lista, con confirmación; los mensajes se van solos por FK `on delete cascade`) y **borrar un mensaje individual** (hover en desktop / long-press en mobile, con confirmación). RLS: solo el dueño borra lo suyo; la lista y el chat se actualizan al instante (Realtime + optimista).
  - **Pantalla completa (ajustado 17/07):** la Bandeja ocupa casi toda el área de contenido (clase `main-flush` en `App.tsx` cuando la vista es `bandeja`): lista izquierda con ancho fijo (~340px), chat a la derecha, burbujas con `max-width` acotado (no se estiran en pantallas anchas), mensajes con scroll propio y composer fijo abajo. Responsive: en mobile full-bleed, se ve la lista y al tocar una charla se abre el chat a pantalla completa con botón de volver.

- **Capa 4 — LAS MANOS, tanda 1 (LEER) (HECHO 17/07, working tree):** el agente dejó de tener el catálogo entero en el prompt y ahora lo **consulta con herramientas** (tool use de Anthropic, ejecutadas server-side con el `user_id` del token + RLS; el loop suma los tokens de todas las llamadas del turno). El system prompt ahora lleva solo un **resumen** (nombre, rubro, horarios, cuántos productos/servicios hay y sus categorías) y le dice al agente que use las tools para el detalle. Herramientas (solo LECTURA, no modifican nada):
  - `buscar_productos(query, categoria?)` — busca por nombre/categoría/descripción, devuelve nombre/precio/stock/categoría (máx 10).
  - `consultar_stock(nombre_producto)` — stock real de un producto.
  - `buscar_servicios(query?, categoria?)` — servicios con precio (o "a consultar") y duración.
  - `consultar_disponibilidad(fecha, hora?, servicio?, empleado?)` — usa las RPC ya verificadas (`check_availability` / `agenda_free_slots`); NO reserva, solo consulta.
  - `buscar_empleados(query?)` — equipo con nombre, rol y qué servicios hace.
  - **Contexto temporal:** el prompt incluye la **fecha y hora actual en zona horaria de Argentina** (calculada en el server por request, con `Intl` — nada de UTC directo). Así el agente resuelve "hoy/mañana/pasado/el viernes/la semana que viene" a la fecha concreta y le habla al cliente en lenguaje natural ("para mañana martes 16").
  - **Match de nombres flexible:** los empleados se buscan **sin tildes ni mayúsculas** (helper `fold()`, mismo criterio que el buscador global); si hay varios que coinciden pide al cliente cuál, y si no existe ofrece los que SÍ están (no manda a llamar por teléfono a lo bobo). Todavía no tolera errores de tipeo tipo b/v ("labador" ≠ "lavador").
  - **Resuelve el "problema del catálogo grande":** ya no viaja el catálogo entero a la IA en cada mensaje → mucho más barato y escala a negocios con miles de productos. **Falta redeployar** cada cambio de la function con `npx supabase functions deploy chat`.
- **Menú reorganizado (17/07):** **Conversaciones** y **Canales** salieron del grupo "Servicios" (que es de gestión: Agenda, Equipo) a un grupo nuevo **"Atención"**, ubicado arriba, cerca de lo operativo del día a día.
- **Lo que falta del cerebro:** **capa 4 tanda 2 (las manos que ESCRIBEN: agendar/reservar turnos de verdad)**, **capa 5 (panel de tokens/costo)** que lea lo que ya se guarda, la **derivación inteligente a humano**, la **fase multimodal** (audios/fotos/comprobantes) y el **entrenador conversacional**, más conectar los canales reales encima de la misma bandeja. Ver 6.1.
...
> 4. **Las manos (tools):**
>    - **Tanda 1 — LEER: ✅ HECHO (17/07).** El agente CONSULTA la base con herramientas: `buscar_productos`, `consultar_stock`, `buscar_servicios`, `consultar_disponibilidad` (RPC ya verificada, solo consulta) y `buscar_empleados`. No inventa: responde con datos reales, y si la tool no encuentra nada lo dice. Sabe fecha/hora de Argentina (resuelve "mañana", "el viernes", "la semana que viene") y matchea empleados sin tildes/mayúsculas. Ver 3.10.
>    - **Tanda 2 — ESCRIBIR: ⏳ PENDIENTE.** Que el agente **agende/reserve el turno de verdad** (escribir en `appointments`), tome datos del cliente, etc. Acá termina de conectarse todo lo construido. Este contexto temporal y las tools de consulta ya son la base.
>    - **⚠️ PROBLEMA DEL CATÁLOGO GRANDE (detectado 15/07) — RESUELTO en la tanda 1:**
>      - **(a) Comportamiento:** el agente NO vuelca toda la lista de una; pregunta qué busca y recomienda como vendedor, y solo lista todo si el cliente lo pide. → Reforzado en el prompt (sección "actitud comercial", ver 3.10).
>      - **(b) Técnico: ✅ RESUELTO.** Antes el catálogo entero iba en el system prompt (viajaba a la IA en CADA mensaje = carísimo, inviable con 1000 productos). Ahora el prompt lleva solo un resumen (count + categorías) y el agente **busca con tools** lo que necesita. Escala a catálogos grandes y abarata cada conversación.
